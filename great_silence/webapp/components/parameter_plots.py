...
DARK_THEME = dict(
    ...
    hoverlabel=dict(
        bgcolor="rgba(15,18,32,0.92)",
        bordercolor="rgba(255,255,255,0.14)",
        font=dict(family="Inter, ui-sans-serif, system-ui, sans-serif", color="#e8edf6", size=11),
    ),
    xaxis=dict(
...
class ParameterPlots:
    ...
    def _build(self) -> None:
        with ui.card().classes("w-full"):
            ui.label("Parameter Visualization").classes("gs-eyebrow text-gray-300 mb-2").style(
                "font-size: 0.85rem;"
            )
    ...
    def _create_kardashev_plot(self) -> go.Figure:
        ...
        crisis_peaks = [
            (
                ...
                0.72,
                0.05,
                config.crisis_nuclear_age_amplitude,
                config.enable_nuclear_crisis,
                COLORS["orange"],
            ),
            (
                ...
                0.85,
                0.05,
                config.crisis_planetary_unification_amplitude,
                config.enable_planetary_unification_crisis,
                COLORS["yellow"],
            ),
            (
                "AI Transition",
                1.05,
                0.08,
                config.crisis_ai_transition_amplitude,
                config.enable_ai_crisis,
                COLORS["red"],
            ),
            (
                "Interplanetary",
                1.25,
                0.10,
                config.crisis_interplanetary_amplitude,
                config.enable_interplanetary_crisis,
                COLORS["purple"],
            ),
            (
                "Stellar Engineering",
                1.80,
                0.15,
                config.crisis_stellar_engineering_amplitude,
                config.enable_stellar_crisis,
                COLORS["cyan"],
            ),
            (
                ...
                2.50,
                0.20,
                config.crisis_relativistic_weapons_amplitude,
                config.enable_relativistic_weapons_crisis,
                COLORS["pink"],
            ),
        ]
    ...
    def _create_imf_plot(self) -> go.Figure:
        ...
        if imf_type == "kroupa":
            ...
            m_break = 0.5
            imf = np.where(
                masses < m_break, masses**alpha1, (m_break ** (alpha1 - alpha2)) * masses**alpha2
            )
