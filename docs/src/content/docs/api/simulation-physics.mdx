---
title: "Physics"
description: "API reference for great_silence.simulation.physics"
sidebar:
  order: 27
---
...
**Module:** `great_silence.simulation.physics`

Physics calculations for light travel time and relativistic effects.

```python
from great_silence.simulation.physics import ...
```
...
##### `light_travel_time` `staticmethod`
...
##### `relativistic_time_dilation` `staticmethod`
...
```python
def proper_time(coordinate_time_yr: float, velocity_fraction_c: float) -> float
```
...
##### `observable_horizon` `staticmethod`
...
```python
def observable_horizon(current_time_yr: float, observer_position: np.ndarray, stellar_positions: np.ndarray) -> np.ndarray
```
