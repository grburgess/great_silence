---
title: "Probe Design"
description: "API reference for great_silence.civilization.probe_design"
sidebar:
  order: 10
---
...
**Module:** `great_silence.civilization.probe_design`
...
```python
def min_metallicity_for_replication(kardashev_level: float, threshold_k085: float = -0.3, threshold_k095: float = -0.5, threshold_k120: float = -1.0) -> float
```

**Parameters:**

- `kardashev_level` (`float`)
- `threshold_k085` (`float`) — default: `-0.3`
- `threshold_k095` (`float`) — default: `-0.5`
- `threshold_k120` (`float`) — default: `-1.0`
